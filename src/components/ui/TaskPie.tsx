...
// ✅ Define LABELS and COLORS outside component (static, avoids re-creation)
...
export default function TaskPie({
  ...
  const data: ChartData<"pie", number[], string> = useMemo(
    () => ({
      labels: [...LABELS], // spread ensures a plain array
      datasets: [
        {
          label: "Count",
          data: values,
  ...
  return (
    <Card className="p-4">
      ...
      <div className="w-full flex flex-col sm:flex-row items-center text-center gap-3 sm:gap-4">
        ...
        {/* Legend */}
        <ul
          className="text-xs text-gray-700 space-y-2 sm:min-w-[160px]"
          role="list"
