...
export async function GET(req: NextRequest) {
  ...
  if (!divisionId) return Response.json({ error: 'divisionId is required' }, { status: 400 });
  const framework = params.get('framework');
  const year = params.get('year');
  const qs = new URLSearchParams();
  qs.set('divisionId', divisionId);
  if (framework) qs.set('framework', framework);
  if (year) qs.set('year', year);
  const url = `${base}/api/assessments/progress/compliance/by-branch?${qs.toString()}`;
