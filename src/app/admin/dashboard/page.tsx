...
export default function AdminDashboardPage() {
  ...
  // UI: expand/collapse per division
  const [expanded, setExpanded] = useState<string | null>(null);

  useEffect(() => {
    let cancelled = false;

    // Frameworks (if needed later)
    (async () => {
      try {
        setFwLoading(true);
        ...
        if (!cancelled) setFwError(err instanceof Error ? err.message : "Failed to load frameworks");
      } finally {
        if (!cancelled) setFwLoading(false);
      }
    })();

    // Hierarchical progress (division + branches + per-framework)
    (async () => {
      try {
        const res = await fetch('/api/assessments/progress/by-division-hier', { cache: 'no-store', credentials: 'include' });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const payload: DivisionHierarchyResponse = await res.json();
        if (!cancelled) setHier(payload);
      } catch (e) {
        if (!cancelled) setHierErr(e instanceof Error ? e.message : 'Failed to load hierarchical progress');
      }
    })();
    ...
    (async () => {
      try {
        const res = await fetch('/api/assessments/completed', { cache: 'no-store', credentials: 'include' });
        if (res.ok) {
  ...
  const metrics = useMemo(() => {
    return [
      { title: "Active Assessments", value: String(hier?.overallActiveAssessments ?? 0), subtitle: "Ongoing across divisions" },
      { title: "Completed", value: String(completedCount), subtitle: "Assessments closed" },
    ];
  }, [hier?.overallActiveAssessments, completedCount]);

  const renderBranchPies = (branch: DivisionHierarchyResponse["divisions"][number]["branches"][number]) => {
    const fws = branch.frameworks ?? [];
    if (fws.length >= 2) {
      // Look for ISO/GDPR specifically; fallback to first two if names differ
      ...
      return (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <TaskPie label={`${branch.branchName} · ${iso.name}`} completed={iso.totals.completed} pending={iso.totals.pending} notCompleted={iso.totals.notCompleted} />
          <TaskPie label={`${branch.branchName} · ${gdpr.name}`} completed={gdpr.totals.completed} pending={gdpr.totals.pending} notCompleted={gdpr.totals.notCompleted} />
        </div>
      );
    }
    if (fws.length === 1) {
      const fw = fws[0];
      return <TaskPie label={`${branch.branchName} · ${fw.name}`} completed={fw.totals.completed} pending={fw.totals.pending} notCompleted={fw.totals.notCompleted} />;
    }
    // No framework breakdown: use branch totals
    return <TaskPie label={branch.branchName} completed={branch.totals.completed} pending={branch.totals.pending} notCompleted={branch.totals.notCompleted} />;
  };
    ...
    <div className="flex flex-col space-y-8 min-h-full overflow-auto px-3 sm:px-4 md:px-6 py-4 bg-gradient-to-b from-gray-100 via-gray-200 to-gray-300">
      ...
      <section className="p-4 bg-white rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition mb-4">
        <div className="flex justify-between items-center mb-3">
          ...
          <a href="/dashboard/news" className="text-sm text-indigo-600 hover:underline">View More →</a>
        </div>
      ...
      {/* Top Metrics (Active + Completed) */}
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 min-w-0">
        {metrics.map((m) => (
          <Card key={m.title} className="bg-white shadow-md rounded-xl border border-gray-100 hover:scale-[1.03] hover:shadow-xl transition-all duration-300">
            <CardHeader>
            ...
            <CardContent>
              ...
              {m.subtitle && <p className="text-xs md:text-sm text-gray-500 mt-1">{m.subtitle}</p>}
            </CardContent>
          </Card>
        ))}
      </section>

      {/* Division (radial) + Branch pies (expandable) */}
      <section className="pt-2">
        <h2 className="text-lg md:text-xl font-bold text-gray-900 mb-4">Division & Branch Progress</h2>
        ...
        <div className="space-y-8">
          {hier?.divisions?.map((d) => (
            <div key={d.divisionId}>
              <button onClick={() => setExpanded(expanded === d.divisionId ? null : d.divisionId)} className="w-full text-left">
                <RadialProgress label={d.divisionName} percent={d.completionPercent ?? 0} />
              </button>
